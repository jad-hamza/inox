...
private[ast] trait SymbolTransformerComposition extends SymbolTransformer {
  ...
  override def transform(syms: s.Symbols): t.Symbols = {
    val r = 
      if (rhs.name.isEmpty) rhs.transform(syms)
      else Bench.time("Symbol Transformer (" + rhs.name.get + ")", rhs.transform(syms))
    if (lhs.name.isEmpty) lhs.transform(r)
    else Bench.time("Symbol Transformer (" + lhs.name.get + ")", lhs.transform(r))
  }
